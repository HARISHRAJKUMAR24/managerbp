...
if ($generated_signature === $signature) {
    ...
    if ($tableExists) {
        try {
            ...
            // If insert fails, still return success but with a temporary invoice number
            $invoice_number = time(); // Use timestamp as temporary invoice
            
            echo json_encode([
                "success" => true,
                "message" => "Payment verified successfully (record not saved: " . $e->getMessage() . ")",
                "invoice_number" => $invoice_number,
                "payment_id" => $payment_id,
                "redirect_url" => "/payment-success?invoice=" . $invoice_number
            ]);
        ...
        // Table doesn't exist, create a temporary invoice number
        $invoice_number = time(); // Use timestamp as temporary invoice
        ...
        echo json_encode([
            ...
            "payment_id" => $payment_id,
            "redirect_url" => "/payment-success?invoice=" . $invoice_number,
            "note" => "Database table not found, using temporary invoice number"
        ]);
    }
